...
export class IndiceUsuariosComponent {
  ...
  serviciosSeguridad = inject(SeguridadService);
  ...
  cargarRegistros(){
    this.serviciosSeguridad.obtenerUsuarioPaginado(this.paginacion)
    .subscribe(respuesta =>{
  ...
  hacerAdmin(email: string){
    this.serviciosSeguridad.hacerAdmin(email).subscribe(() => {
      Swal.fire("Exitoso", `El usuario ${email} ahora es admin`, 'success')
    })
  }
  removerAdmin(email: string){
    this.serviciosSeguridad.removerAdmin(email).subscribe(() => {
